...
void GameScreen::Draw()
{
	...
	_player.UpdateInputs(GetFrameTime());

	if (IsKeyDown(KEY_A) && player_anim.img.width > 0){
		player_anim.FLipTextureHoriz();
	}
	if (IsKeyDown(KEY_D) && player_anim.img.width < 0){
		player_anim.FLipTextureHoriz();
	}

	player_anim.CheckAnimationState();
